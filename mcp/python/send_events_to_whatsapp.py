#!/usr/bin/env python3
"""
Send all TODAY events to WhatsApp
"""
...
async def send_today_events_whatsapp():
    print("=" * 70)
    print("Fetching Today's Events and Sending to WhatsApp")
    print("=" * 70)
    ...
    try:
        ...
        if not unique_reminders:
            ...
            for i, reminder in enumerate(unique_reminders, 1):
                ...
                if notes:
                    message += f"   {notes}\n"
                message += f"   Time: {due_date_time}\n"
                if recurrence != "none":
                    message += f"   Recurring: {recurrence}\n"
                message += "\n"
        
        print("\n3. Initializing WhatsApp notifier...")
        
        notifier = WhatsAppNotifier()
        await notifier.initialize()
        print("   ✓ WhatsApp notifier initialized")
        
        print("\n4. Sending to WhatsApp...")
        success = await notifier.send_message(message)
        
        if success:
            print("   ✓ Message sent successfully!")
            print("\n" + "=" * 70)
            print("✓ SUCCESS: Today's events sent to WhatsApp")
            print("=" * 70)
            return True
        else:
...
if __name__ == "__main__":
    asyncio.run(send_today_events_whatsapp())
